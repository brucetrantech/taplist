...
import { DATE_FORMATTER, priorityOrder, PriorityValues } from '@/commons/constants';
...
export function getPriorityValue (priority: Priority): string[] {
    ...
    return [
        value.label,
        value.color || themes.color.gray.level1,
    ];
}

export function formatDate (date: Date, formatter: string = DATE_FORMATTER): string {
...
export function sortTaskByPriority(tasks: Array<Task> = []): Array<Task> {
    ...
    return update.sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
}
